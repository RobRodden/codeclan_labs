...
```{r}
pattern_how_many_times_big_brother <- "(?i)[b]+[b]"

whole_text %>%
  str_extract_all(pattern_how_many_times_big_brother)
```
...
```{r}
whole_text %>% 
  str_extract("^[A-Z]{1}[a-z]+[0-9]$")
```

```{r}

pattern <- "Winston Smith|Winston"
...
```{r}
whole_text %>% 
  str_replace_all("[:space:]gin[:space:]", "vodka")
```
...
```{r}
whole_text %>% 
  str_extract_all("(?i)[:space:]gin[[:space:][:punct:]]", "vodka")
```
