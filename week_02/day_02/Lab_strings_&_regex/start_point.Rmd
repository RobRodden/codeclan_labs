...
```{r}
whole_text %>%
  str_extract("[A-Z]{1}[a-z]+[:space:][0-9]th[:punct:][:space:][0-9]+")
...
```{r}
whole_text %>%
  str_extract("[A-Z]{1}[a-z]+ [0-9]{1,2}[a-z]{2}, [0-9]{4}")
...
```{r}
pattern <- "(?i)Winston Smith|Winston"
...
or this (which is less effective)
```{r}
pattern_alt <- "Winston Smith|Winston"

whole_text_redacted_alt <- whole_text %>%
  str_replace_all(pattern, "REDACTED")

whole_text_redacted_alt
...
```{r}
# identify what needs to changed.
pattern1 <- "[ ,\\.-][Gg][Ii][Nn][ , \\.-]"

whole_text_extract <- whole_text %>%
...
```{r}
# make the changes.
whole_text_replaced <- whole_text %>%
  str_replace_all(c(" GIN." = " VODKA.",
                    " gin." = " vodka.",
                    " gin " = " vodka "))

# check the changes have worked.
whole_text_extract <- whole_text_replaced %>%
  str_extract_all(pattern1) %>%
  list_c()
...
```{r}
# identify what needs to changed.
pattern2 <- "Department|Ministry|Ministrie|Mini"

whole_text_extract <- whole_text %>%
  str_extract_all(pattern2) %>%
  list_c()

unique(whole_text_extract)

```

```{r}
# make the required changes.
whole_text_test <- whole_text %>%
  str_replace_all(c("Department" = "Section",
                    "Ministry" = "Department",
                    "Ministrie" = "Department",
                    "Mini" = "Dep"))

# confirm changes
pattern2_alt <- c("Section", "Department", "Dep")

whole_text_test2 <- whole_text_test %>%
  str_extract_all(pattern2_alt) %>%
  list_c()

unique(whole_text_test2)
```
...
Stocks of Victory Gin have run out and the people of Oceania will have to drink vodka instead. Replace “gin” with “vodka”, matching case wherever possible. There’s still loads of Victory Coffee, though, so don’t replace that.
```{r}
# again, identify what needs changed.
pattern3 <- "[ ,\\.-][Gg][Ii][Nn][ ,\\.-]"
...
Sample text to check function correctly replaces identified text.
```{r}
text_sample <- " GIN. gin. gin VICTORY GIN and Victory Gin. Gingersnap"

```

Function to perform case-sensitive replacements of 'gin' and 'Victory Gin'
```{r}
replace_gin <- function(text) {
  # Define pattern to match standalone 'gin'/'GIN' or mixed case
  pattern <- "\\b[gG][iI][nN]\\b"

  # Find all matches of the pattern
  match_positions <- gregexpr(pattern, text, perl = TRUE, ignore.case = FALSE)

  # Extract matches
  matches <- regmatches(text, match_positions)[[1]]

  # Replace each match based on case
  replacements <- sapply(matches, function(match) {
    if (grepl("^[A-Z]+$", match)) {
      "VODKA"  # Replace uppercase "GIN" or "Victory GIN"
    } else if (grepl("^[a-z]+$", match)) {
      "vodka"  # Replace lowercase "gin" or "Victory gin"
    } else if (grepl("^[A-Z][a-z]+$", match)) {
      "Vodka"  # Replace capitalized "Gin" or "Victory Gin"
    }
  })

  # Replace matches in original text
...
# Run the function
whole_text_gin_replaced <- replace_gin(whole_text)

# Display result
cat(whole_text_gin_replaced)
...
Big Brother is rebranding and has decided that the various ministries will now be known as departments. The old departments are now called sections. The Newspeak titles will use “Dep” instead of “Mini”.
```{r}
# test text to check that it's working (commented out)
# test_text2 <- "Department Ministry Ministrie Mini"

# function for task
replace_words <- function(text, replacements) {
  for (i in seq_along(replacements)) {
    text <- str_replace_all(text, names(replacements)[i], replacements[i])
  }
  return(text)
}

# replace matches in original text
replacements <- c(Department = "Section", Ministry = "Department", Ministrie = "Department", Mini = "Dep")

whole_text2 <- whole_text %>%
